<template>
  <el-form-item :label="widget.name" :prop="widget.model">
    <template v-if="widget.type == 'input'">
      <el-input v-if="widget.options.dataType == 'number' || widget.options.dataType == 'integer' || widget.options.dataType == 'float'" :type="widget.options.dataType" v-model.number="dataModel" :placeholder="widget.options.placeholder" :style="{width: widget.options.width}"></el-input>
      <el-input v-else :type="widget.options.dataType" v-model="dataModel" :placeholder="widget.options.placeholder" :style="{width: widget.options.width}"></el-input>
    </template>

    <template v-if="widget.type == 'textarea'">
      <el-input type="textarea" :rows="5" v-model="dataModel" :disabled="widget.options.disabled" :placeholder="widget.options.placeholder" :style="{width: widget.options.width}"></el-input>
    </template>

    <template v-if="widget.type == 'number'">
      <el-input-number v-model="dataModel" :style="{width: widget.options.width}" :step="widget.options.step" controls-position="right"></el-input-number>
    </template>
    ...
    <template v-if="['signupName','signupEmail','signupPhone','signupCompany','signupPost','employeeNum'].indexOf(widget.type) !== -1">
      <el-input :type="widget.options.dataType" v-model="dataModel" :placeholder="widget.options.placeholder" :style="{width: widget.options.width}"></el-input>
    </template>

    <template v-if="widget.type == 'gender'">
      <el-radio-group v-model="dataModel" :style="{width: widget.options.width}">
        <el-radio :style="{display: widget.options.inline ? 'inline-block' : 'block'}" :label="item.value" v-for="(item, index) in widget.options.options" :key="index">{{item.label}}</el-radio>
      </el-radio-group>
    </template>

    <!-- 地址 -->
    <template v-if="['companyAddr','homeAddress'].indexOf(widget.type) !==-1">
      <!-- {{models}} -->
      <area-cascader ref="areaCascader" :data.sync="widget.options" @handleChange="cascaderChange" @onBlur="onBlur" ></area-cascader>
      <!-- :placeholder="widget.options.placeholder" -->
      <!-- <el-input
        :type="widget.options.dataType"
        v-model="widget.options.detailsAddress"
        :style="{width: widget.options.width}"
      ></el-input>-->
    </template>
  </el-form-item>
...
export default {
  ...
  created () {
    ...
    //   })
    // }
    if (['companyAddr', 'homeAddress'].indexOf(this.widget.type) !== -1) {
      this.models[this.widget.type + 'Details'] = ''
    }
  },
  methods: {
    cascaderChange (e) {
      this.models[this.widget.type] = e
      // this.models.companyAddress = {
      //   provinceId: e[0],
      //   cityId: e[1],
      //   countyId: e[2]
      // }
    ...
    onBlur (e) {
      this.models[this.widget.type + 'Details'] = e
    }
  },
  watch: {
    dataModel: {
