  ...
  <div v-if="show">
    <el-form label-position="top">
      <el-form-item label="标题" v-if="data.type!='grid'">
        <el-input v-if="['signupName','gender','signupEmail','signupPhone','signupCompany','signupPost','companyAddr','homeAddress','employeeNum'].indexOf(data.type) !== -1" v-model="data.name" readonly></el-input>
        <el-input v-else v-model="data.name"></el-input>
      </el-form-item>
      ...
      <el-form-item label="选项" v-if="Object.keys(data.options).indexOf('options')>=0&&data.type!=='gender'">
            ...
            <el-radio-group v-model="data.options.defaultValue">
              <draggable element="ul" :list="data.options.options" :options="{group:{ name:'options'}, ghostClass: 'ghost',handle: '.drag-item'}">
                <li v-for="(item, index) in data.options.options" :key="index">
                  ...
                  <i class="drag-item" style="font-size: 16pxmargin: 0 5pxcursor: move">
                    <i class="iconfont icon-icon_bars"></i>
                  </i>
                  <el-button @click="handleOptionsRemove(index)" circle plain type="danger" size="mini" icon="el-icon-minus" style="padding: 4pxmargin-left: 5px"></el-button>
                </li>
              </draggable>
            </el-radio-group>
          </template>
      ...
      <el-form-item label="默认值" v-if="Object.keys(data.options).indexOf('defaultValue')>=0 && (data.type == 'textarea' || data.type == 'input' || data.type=='rate' || data.type=='color' || data.type=='switch')">
        ...
        <el-rate v-if="data.type == 'rate'" style="display:inline-blockvertical-align: middle" :max="data.options.max" :allow-half="data.options.allowHalf" v-model="data.options.defaultValue"></el-rate>
        <el-button type="text" v-if="data.type == 'rate'" style="display:inline-blockvertical-align: middlemargin-left: 10px" @click="data.options.defaultValue=0">清空</el-button>
      ...
      <template v-if="data.type == 'time' || data.type == 'date'">
        ...
        <el-form-item label="默认值" v-if="data.type=='time' && Object.keys(data.options).indexOf('isRange')>=0">
          <el-time-picker key="1" style="width: 100%" v-if="!data.options.isRange" v-model="data.options.defaultValue" :arrowControl="data.options.arrowControl" :value-format="data.options.format"></el-time-picker>
          <el-time-picker key="2" v-if="data.options.isRange" style="width: 100%" v-model="data.options.defaultValue" is-range :arrowControl="data.options.arrowControl" :value-format="data.options.format"></el-time-picker>
        </el-form-item>
      </template>
      ...
      <template v-if="['grid','signupName','gender','signupEmail','signupPhone','signupCompany','signupPost','companyAddr','homeAddress','employeeNum'].indexOf(data.type) == -1">
      ...
      <template v-if="['signupName','gender','signupEmail','signupPhone','signupCompany','signupPost','companyAddr','homeAddress','employeeNum'].indexOf(data.type) !== -1">
        <el-form-item label="字段名">
        ...
        <el-form-item label="校验">
          ...
          <div v-if="Object.keys(data.options).indexOf('pattern')>=0">
            <el-input size="mini" v-model.lazy="data.options.pattern" style=" width: 240px" placeholder="填写正则表达式" readonly>{{data.pattern}}</el-input>
          </div>
        </el-form-item>
      </template>
    </el-form>
  ...
  watch: {
    ...
    'data.options.required': function (val) {
      if (val) {
        ...
        if (['companyAddr', 'homeAddress'].indexOf(this.data.type) !== -1) {
          this.validator.required.validator = (rules, value, callback) => {
            if (value.length <= 0) {
              callback(new Error('详细地址必须填写'))
            } else if (this.data.options.detailsAddress && this.data.options.detailsAddressDesc === '') {
              callback(new Error('详细地址必须填写'))
              this.validator.required.message = '详细地址必须填写'
            } else {
              callback()
            }
          }
    ...
    'data': function (data) {
      ...
      if (['companyAddr', 'homeAddress'].indexOf(data.type) !== -1) {
        if (!data.options.required) {
          return false
        }
        if (data.options.detailsAddress) {
          this.validator.detailsAddress = {
            validator: (rules, value, callback) => {
              if (!data.options.required) {
                callback()
              }
              if (data.options.detailsAddressDesc === '') {
                callback(new Error('详细地址必须填写'))
              } else {
                callback()
              }
          ...
          this.validator.detailsAddress = null
        }
      }
    },
