<template>
  <el-form-item class="widget-view" v-if="element && element.key" :class="{active: selectWidget.key == element.key, 'is_req': element.options.required}" :label="element.name" @click.native.stop="handleSelectWidget(index)">
    ...
    <template v-if="element.type == 'radio'">
      <el-radio-group v-model="element.options.defaultValue" :style="{width: element.options.width}">
        <el-radio :style="{display: element.options.inline ? 'inline-block' : 'block'}" :label="item.value" v-for="(item, index) in element.options.options" :key="item.value + index">{{element.options.showLabel ? item.label : item.value}}</el-radio>
      </el-radio-group>
    </template>

    <template v-if="element.type == 'checkbox'">
      <el-checkbox-group v-model="element.options.defaultValue" :style="{width: element.options.width}">
        <el-checkbox :style="{display: element.options.inline ? 'inline-block' : 'block'}" :label="item.value" v-for="(item, index) in element.options.options" :key="item.value + index">{{element.options.showLabel ? item.label : item.value}}</el-checkbox>
      </el-checkbox-group>
    </template>

    <template v-if="element.type == 'time'">
      <el-time-picker v-model="element.options.defaultValue" :is-range="element.options.isRange" :placeholder="element.options.placeholder" :start-placeholder="element.options.startPlaceholder" :end-placeholder="element.options.endPlaceholder" :readonly="element.options.readonly" :disabled="element.options.disabled" :editable="element.options.editable" :clearable="element.options.clearable" :arrowControl="element.options.arrowControl" :style="{width: element.options.width}"></el-time-picker>
    </template>
    ...
    <template v-if="['signupName','signupEmail','signupPhone','signupCompany','signupPost','employeeNum'].indexOf(element.type) !== -1">
      <el-input v-model="element.options.defaultValue" companyAddress :placeholder="element.options.placeholder"></el-input>
    </template>

    <template v-if="element.type == 'gender'">
      <el-radio-group v-model="element.options.defaultValue" :style="{width: element.options.width}">
        <el-radio :style="{display: element.options.inline ? 'inline-block' : 'block'}" :label="item.value" v-for="(item, index) in element.options.options" :key="item.value + index">{{item.label}}</el-radio>
      </el-radio-group>
    </template>
    <!-- 地址 -->
    <template v-if="['companyAddr','homeAddress'].indexOf(element.type)!==-1">
      <!-- v-model="element.options.defaultValue" -->
      <area-cascader :data.sync="element.options"></area-cascader>
    </template>
    ...
    <el-button title="复制" @click.stop="handleWidgetClone(index)" class="widget-action-clone" v-if="selectWidget.key == element.key&&['signupName','gender','signupEmail','signupPhone','signupCompany','signupPost','companyAddr','homeAddress','employeeNum'].indexOf(element.type) == -1" circle plain type="primary">
...
import { commonComponents } from './componentsConfig.js'
export default {
    ...
    handleWidgetDelete (index) {
      if (
        [
          ...
          'homeAddress',
          'employeeNum'
        ].indexOf(this.data.list[index].type) !== -1
      ) {
        const type = this.data.list[index].type
        const match = commonComponents.filter((item, index) => {
          return item.type === type
        })
        match[0].everUsed = false
      }
